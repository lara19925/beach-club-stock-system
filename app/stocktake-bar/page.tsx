...
export default function BarStocktakePage() {
  ...
  const filteredRows = rows.filter((row) => {
    const matchesSearch =
      ...
      row.plu_code.toLowerCase().includes(search.toLowerCase())
  ...
  function updateRow(
    pluCode: string,
    field: 'sealed_qty' | 'open_weight_kg' | 'unit_qty',
    value: number
  ) {
    setRows((prev) =>
      prev.map((row) => {
        if (row.plu_code !== pluCode) return row
        ...
        if (updated.count_type === 'UNIT') {
          updated.total_weight_kg = Number(updated.unit_qty || 0)
        } else {
          updated.total_weight_kg =
            Number(updated.sealed_qty || 0) *
              Number(updated.full_bottle_weight_kg || 0) +
            Number(updated.open_weight_kg || 0)
        }
  ...
  async function saveStocktake(status: 'DRAFT' | 'FINAL') {
    ...
    const countedRows = rows.filter(
      (row) => Number(row.total_weight_kg || 0) > 0
    )
  ...
  function exportCSV() {
    ...
    const csvRows = filteredRows
      ...
      .map((row) => [row.plu_code, row.total_weight_kg.toFixed(3), 2])
